const domain = process.env.SHOPIFY_STORE_DOMAIN; // change to url found on frontpage of the project report
const storefrontAccessToken = process.env.SHOPIFY_STOREFRONT_ACCESSTOKEN;  // change to token found on frontpage of the project report
...
// getting products from our Home Page collection with our created query
...
// for future sorting of products by price, new to old etc.
export async function getProductsbyCollection(handle) {
	...
	const productsByCollection = response.data.collectionByHandle?.products.edges
		? response.data.collectionByHandle.products.edges
		: [];
